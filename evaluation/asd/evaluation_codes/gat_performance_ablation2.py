"""
ablation_gat_stats.py
==============================================================
Ablation study to prove TriFusion > individual components.
Runs 30-fold LOSO on the same 15+15 balanced dataset.

Models compared:
  1. GAT Only       — scanpath graph, no CNN, no stats
  4. TriFusion Ref  — reference number from your main run

Same LOSO protocol, same seed, same data as multiview code.
Use results to fill your ablation table in the paper.
"""
...
# ── CONFIG (identical to your multiview code) ──────────────────
SEED         = 42
...
# ── DATA LOADING (identical to your code) ─────────────────────
...
# ══════════════════════════════════════════════════════════════
# ABLATION MODEL 1: GAT ONLY
# Input: scanpath graph only
# No CNN features, no stats
# ══════════════════════════════════════════════════════════════
class GATOnly(nn.Module):
    ...
    def forward(self, graph, stats=None, cnn=None):
        ...
        x  = self.norm(global_mean_pool(x, graph.batch))
        return self.classifier(x)


# ══════════════════════════════════════════════════════════════
# ABLATION MODEL 2: STATS ONLY
# Input: 16 clinical features only
# No CNN features, no GAT
# ══════════════════════════════════════════════════════════════
class StatsOnly(nn.Module):
    ...
    def forward(self, graph=None, stats=None, cnn=None):
        return self.net(stats)


# ══════════════════════════════════════════════════════════════
# ABLATION MODEL 3: GAT + STATS (no CNN)
# Stats acts as attention query over GAT embedding
# Same cross-modal gate as TriFusion, just no CNN key/value
# ══════════════════════════════════════════════════════════════
class GATplusStats(nn.Module):
    def __init__(self):
        ...
        # Stats projection (query)
        self.stat_proj = nn.Sequential(
            nn.Linear(STAT_DIM, H), nn.LayerNorm(H), nn.GELU()
        )
        # Simple gating: stats-weighted GAT
    ...
    def forward(self, graph, stats, cnn=None):
        # GAT
        x = F.elu(self.gat1(graph.x, graph.edge_index, graph.edge_attr))
        x = F.elu(self.gat2(x,       graph.edge_index, graph.edge_attr))
        x = self.gat3(x,             graph.edge_index, graph.edge_attr)
...
def run_loso(model_class, model_name, all_data):
    ...
    for fold_i, (test_cls, test_subj) in enumerate(subject_keys):
        ...
        w     = torch.tensor([1.0, n_td / max(n_asd, 1)],
...
def summarize(fold_results, model_name):
    ...
    avg_prob_asd  = float(np.mean([r['avg_prob'] for r in fold_results
                                   if r['true_label'] == 1]))
    avg_prob_td   = float(np.mean([r['avg_prob'] for r in fold_results
                                   if r['true_label'] == 0]))
    ...
    metrics = {
        ...
        'avg_prob_asd':     avg_prob_asd,  # mean P(ASD) for true ASD subjects
        'avg_prob_td':      avg_prob_td,   # mean P(ASD) for true TD subjects
        'n_correct':        n_correct,
    ...
    print(f"  Avg Conf    : {float(np.mean(confs)):.3f}  "
          f"(correct: {float(np.mean(correct_confs)):.3f}  "
          f"wrong: {f'{float(np.mean(wrong_confs)):.3f}' if wrong_confs else 'n/a'})")
...
def print_comparison_table(all_metrics):
    # TriFusion Full reference — paste your actual numbers here after your main run
    trifusion_ref = {
        'model':       'TriFusion (Full)',
        'accuracy':     0.9667,
        'precision':    1.0000,
        'recall':       0.9333,
        'specificity':  1.0000,
        ...
        'n_correct':    29,
        'n_total':      30,
        'tp': 14, 'tn': 15, 'fp': 0, 'fn': 1,
    }

    rows = all_metrics + [trifusion_ref]
    ...
    for r in rows:
        correct_str   = f"{r['n_correct']}/{r['n_total']}" if 'n_correct' in r else "—"
        avg_prob_asd  = r.get('avg_prob_asd', float('nan'))
        avg_prob_td   = r.get('avg_prob_td',  float('nan'))
    ...
    print(f"  • P(ASD)|ASD >> P(ASD)|TD confirms discriminative power of full model.")
    print("="*100)
...
def main():
    print("="*60)
    print("  Ablation Study: GAT-Only | Stats-Only | GAT+Stats")
    print(f"  Baseline: TriFusion Full = 96.67% (from main run)")
    print("="*60)
